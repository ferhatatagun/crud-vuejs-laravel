...
new Vue ({
  ...
  methods:{
    ...
    createItem: function() {
      ...
      this.$http.post('/vueitems',input).then((response)=> {
        ...
        toastr.success('Post Gönderildi','Başarılı',{timeout:5000});
      },(response)=>
  ...
  deleteITem:function(items){
      ...
      this.changePage(this.pagination.current_page);
      toastr.success('Post Silindi','Başarılı',{timeout:5000});
});
  },
  editItem: function(item){
    this.fillItem.title=item.title;
    this.fillItem.id=item;
  ...
  updateItem:function(id) {
    ...
    this.$http.put('/vueitems/'+id,input).then((response)=> {
      ...
      toastr.success('Post Düzenlendi','Başarılı',{timeout:5000});
    }, (response)=>{
      this.formErrors=response.data;
    });
  },
